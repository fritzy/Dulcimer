...
module.exports = function (mf) {
    ...
    mf.all = function (opts, callback) {
        ...
        laststream = laststream.pipe(new dbstreams.EntryToModelStream(mf, opts.parent, opts.bucket));
    ...
    mf.extendModel({
        ...
        prepJSON: function (withPrivate) {
            var field, fidx;
            var out = this.toJSON({withPrivate: mf.options.savePrivate || withPrivate});
        ...
        __save: function (opts) {
            ...
            var indexes = [];
            var value;
            var outjson = this.prepJSON();
            var outjson_index = this.prepJSON(true);
            for (var fidx in mf.options.bin_indexes) {
                value = outjson_index[mf.options.bin_indexes[fidx]];
            ...
            for (fidx in mf.options.int_indexes) {
                value = outjson_index[mf.options.int_indexes[fidx]];
                if (typeof value === 'undefined') {
                    value = 0;
                }
                indexes.push({
            ...
            if (!this.key) {
                this.getNextKey(opts, function (kerr, key) {
                    ...
                    opts.db.increment('__total__', 1, {bucket: opts.bucket}, function (err, ctr) {
                        opts.db.put('__main__!' + key, outjson, writeopts, function (err, extra) {
                            if (typeof extra === 'object' && extra.hasOwnProperty('vclock')) {
                ...
                opts.db.put('__main__!' + this.key, outjson, writeopts, function (err, vclock) {
