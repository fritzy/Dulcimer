...
module.exports = function (mf) {
        ...
        function gotTotal(err, total, models) {
            ...
            if (models.length > 0) {
                ...
                if (typeof last.__verymeta.extra === 'object') {
                    ...
                    } else if (last.__verymeta.extra.indexKey) {
                        continuation = new Buffer("ik\x00" + last.__verymeta.extra.indexKey).toString('base64');
                    } else if (last.__verymeta.extra.foreignKey) {
                        continuation = new Buffer("fk\x00" + last.__verymeta.extra.foreignKey).toString('base64');
                    }
                }
                if (typeof continuation === 'undefined') {
                    continuation = new Buffer("pk\x00" + '__main__!' + last.key).toString('base64');
                }
            }
    ...
    mf.getTotal = function (opts, callback) {
        ...
        opts.db.get('__total__', {bucket: opts.bucket, type: 'counter'}, function (err, total) {
            if (err || !total) {
                total = 0;
            }
            opts.cb(undefined, total);
        });
    };
    ...
    mf.extendModel({
        ...
        __save: function (opts) {
            ...
            if (!this.key) {
                this.getNextKey(opts, function (kerr, key) {
                    this.key = key;
                    opts.db.put('__total__', 1, {type: 'counter', bucket: opts.bucket}, function (err, ctr) {
        ...
        __delete: function (opts) {
            ...
            opts.db.del('__main__!' + this.key, {bucket: opts.bucket}, function (err) {
                opts.db.put('__total__', -1, {type: 'counter', bucket: opts.bucket, model: mf.options.name}, function (err, ctr) {
