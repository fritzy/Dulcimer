...
module.exports = function (mf) {
    ...
    mf.load = function (key, opts, callback) {
        ...
        if (keylib.getLastChildPrefix(mf, key) !== this.options.name) {
            key = keylib.joinSep(mf, mf.options.name, key);
        }
        opts.db.get(key, {bucket: opts.bucket}, function (err, result) {
            var obj, out;
            if (!err && result) {
    ...
    mf.all = function (opts, callback) {
        ...
        if (opts.db.isRiak) {
            if (streamType === 'base') {
                ...
                laststream = dbstreams.createPrefixEntryReadStream({db: opts.db, prefix: mf.joinSep(opts.prefix, undefined), reverse: opts.reverse, bucket: opts.bucket});
            ...
            if (streamType === 'base') {
                //read in all of the key and values from prefix -> prefix~
                laststream = dbstreams.createPrefixEntryReadStream({db: opts.db, prefix: mf.joinSep(opts.prefix, undefined), reverse: opts.reverse});
            } else if (streamType === 'index') {
                //read all of the index-values of the index
                laststream = dbstreams.createPrefixEntryReadStream({db: opts.db, prefix: mf.joinSep('__index__', opts.prefix, opts.sortBy, undefined), reverse: opts.reverse, index: opts.index});
            } else if (streamType === 'index-value') {
                //read all of the index-values of a specific value
                laststream = dbstreams.createPrefixEntryReadStream({db: opts.db, prefix: mf.joinSep('__index__', opts.prefix, opts.index, opts.indexValue, undefined), reverse: opts.reverse, index: opts.index});
            } else if (streamType === 'index-range') {
        ...
        laststream.pipe(concat(function (models) {
            if (streamType === 'base' || streamType === 'index') {
                ...
                mf.getTotal({db: opts.db, prefix: opts.prefix, bucket: opts.bucket}, function (err, total) {
    ...
    mf.getTotal = function (opts, callback) {
        ...
        if (opts.db.isRiak) {
            opts.db.riak.getCounter({bucket: opts.bucket, key: key}, function (err, reply) {
                var total;
                if (!reply.value) {
                    total = 0;
                } else {
    ...
    mf.extendModel({
        getNextKey: function (opts, callback)  {
            ...
            if (typeof mf.options.keyGenerator !== 'function') {
                increment.incrementKey({db: opts.db, key: keylib.joinSep(mf, '__counter__', prefix), change: 1, bucket: opts.bucket}, function (err, ctr) {
                    var newkey = keylib.joinSep(mf, prefix, keylib.base60Fill(ctr, 10));
                    callback(err, newkey);
                }.bind(this));
            } else {
                mf.options.keyGenerator.call(this, function (err, newkey) {
                    callback(err, keylib.joinSep(mf, prefix, newkey));
                });
            }
        },
        getPrefix: function () {
        ...
        __save: function (opts) {
            if ((!opts.bucket || opts.bucket === 'default') && this.bucket) {
            ...
            if (opts.db.isRiak) {
                ...
                if (!this.key) {
                    this.getNextKey(opts, function (kerr, key) {
                        increment.incrementTotal({db: opts.db, key: keylib.joinSep(mf, '__total__', this.getPrefix()), change: 1, bucket: opts.bucket}, function (err, ctr) {
                            opts.db.put(key, this.prepJSON(), {withVClock: true, indexes: indexes, bucket: opts.bucket}, function (err, vclock) {
                                this.vclock = vclock;
                                this.key = key;
                    ...
                    opts.db.put(this.key, this.prepJSON(), {withVClock: true, vclock: this.vclock, indexes: indexes, bucket: opts.bucket}, function (err, vclock) {
                ...
                async.waterfall([
                    function (acb) {
                        ...
                        if (this.key) {
                            ...
                            this.getNextKey(opts, function (kerr, key) {
                                increment.incrementTotal({db: opts.db, key: keylib.joinSep(mf, '__total__', this.getPrefix()), change: 1}, function (err, ctr) {
                                    acb(kerr, key);
                                });
                            }.bind(this));
                        }
        ...
        __delete: function (opts) {
            if (opts.db.isRiak) {
                opts.db.del(this.key, {bucket: opts.bucket}, function (err) {
                    increment.incrementTotal({db: opts.db, key: keylib.joinSep(mf, '__total__', this.getPrefix()), change: -1, bucket: opts.bucket}, function (err, ctr) {
