    ...
    mf.extendModel({
                    ...
                    if (mf.options.foreignkey_fields.length > 0 && depth > 0) {
                        ...
                        function (field, ecb) {
                            ...
                            if (typeof obj[field] === 'string') {
                                ...
                                if (typeof fmodel === 'undefined') {
                                    throw new Error('Can not find foreign model for foreignKey definition in ' + mf.options.name + '->' + field);
                                }
        ...
        addForeign: function (field, other, opts, callback) {
            ...
            if (Array.isArray(this.bucket)) {
                primaryBucket = this.bucket[0];
            } else {
                primaryBucket = this.bucket;
            }
            var secondaryBucket;
           ...
           if (!secondaryBucket) {
                if (Array.isArray(opts.bucket)) {
                   secondaryBucket = opts.bucket[0];
                } else {
                   secondaryBucket = opts.bucket;
                }
            }
            if (typeof secondaryBucket === 'string') {
                secondaryBucket = [secondaryBucket, mf.options.name];
            }
            if (typeof primaryBucket === 'string') {
                primaryBucket = [primaryBucket, omodel.options.name];
            }
        ...
        hasForeignKey: function (field, other, opts, callback) {
            opts = mf.handleOpts(mf.options.name + '.hasForeign', opts, callback);
            var obj = this;
            var otherkey = other;
            var primaryBucket;
            if (Array.isArray(this.bucket)) {
           ...
           if (!secondaryBucket) {
            ...
            opts.primaryBucket = primaryBucket;
            opts.secondaryBucket = secondaryBucket;
            opts.db.hasForeignKey('__main__!' + this.key, field, '__main__!' + otherkey, opts, opts.cb);
        },
        hasForeign: function (field, other, opts, callback) {
            return this.hasForeignKey(field, other, opts, callback);
        },
        removeForeign: function (field, other, opts, callback) {
            ...
            opts.primaryBucket = [primaryBucket || opts.bucket, mf.options.name];
            opts.secondaryBucket = [secondaryBucket || opts.bucket, omodel.options.name];
            opts.db.delForeignKey('__main__!' + this.key, field, '__main__!' + otherkey, opts, opts.cb);
        },
