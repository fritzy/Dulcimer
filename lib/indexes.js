...
module.exports = function (mf) {
    ...
    mf.extendModel({
        ...
        _saveIndex: function (db, prefix, field, value, callback) {
            async.waterfall([
                function (acb) {
                    increment.incrementKey(db, keylib.joinSep(mf, '__counter__', '__index_value__', prefix, field, value), 1, acb);
                },
                function (count, acb) {
                ...
                function (index, acb) {
                    increment.incrementKey(db, keylib.joinSep(mf, '__total__', '__index_value__', prefix, field, value), 1, function (err) {
                        acb(err, index);
                    });
                },
            ],
        ...
        _deleteIndex: function (db, prefix, oldikey, field, value, callback) {
            async.waterfall([
                ...
                function (acb) {
                    increment.incrementKey(db, keylib.joinSep(mf, '__total__', '__index_value__', prefix, field, value), -1, acb);
                }
            ],
            callback
            );
