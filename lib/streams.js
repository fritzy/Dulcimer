...
function createIndexRangeEntryReadStream(db, start, end, reverse, index) {
    var stream;
    if (reverse) {
        if (index.substr(-4) === '_int') {
            end *= -1;
            start *= -1;
        }
        stream = db.createReadStream({
...
function createPrefixEntryReadStream(db, prefix, reverse, index) {
    var stream;
    if (reverse) {
        stream = db.createReadStream({
            start: prefix + '~',
...
function createIndexPrefixEntryReadStream(db, prefix, reverse, index) {
    var stream;
    if (reverse) {
        stream = db.createReadStream({
            start: prefix,
